...
class FrozenAgentMC_Off_All:
    def __init__(self, env, epsilon: float, p: float, discount_factor: float = 0.95):
        self._epsilon = epsilon
        self._p = p
        self._discount_factor = discount_factor
        self.env = env
    ...
    def initEpisode(self):
        ...
        for state in range(self.env.observation_space.n):
            best_action = np.argmax(self.Q[state])
            #con una probabilidad p cambio la mejor accion por otra al azar
            if np.random.rand()<self.p:
                best_action=np.random.choice(np.arange(self.nA))
